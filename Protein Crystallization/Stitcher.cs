...
namespace Protein_Crystallization
{
    ...
    public class Stitcher : UnmanagedObject
    {
        ...
        public Image<Bgr, Byte> Stitch(Image<Bgr, Byte>[] images)
        {
            ...
            if (ptrs == null)
            {
                throw new ArgumentException("请添加图像");
            }
            ...
            handle.Free();
            //IntPtr p = Marshal.UnsafeAddrOfPinnedArrayElement(ptrs, 0);
            //IntPtr resultIplImage = StitchingInvoke.CvStitcherStitch(_ptr, p, images.Length);
